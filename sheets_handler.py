...
def read_holdings():
    ...
    try:
        ...
        for row in records:
            ...
            # Read target price if exists (column N)
            try:
...
# ── ADD NEW STOCK ─────────────────────────────────────────────
def add_stock_to_holdings(ticker, stock_name, industry, buying_price, buying_date, qty, target_price=None):
    ...
    try:
        ...
        new_row = [
            ...
            buying_date,
            qty,
            investment_amt,   # H = Investment amt
            '',               # I = Current Share Price — updated by updater
            '',               # J = Profit per share — formula
            '',               # K = Total Profit — formula
            '',               # L = Growth — formula
            '',               # M = Investment Days — formula
            target_price if target_price else '',  # N = Target Price
        ]
